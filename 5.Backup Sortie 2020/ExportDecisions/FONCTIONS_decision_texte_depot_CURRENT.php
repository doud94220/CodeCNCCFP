...
class Outil_consultation_sp
{
	...
	*/
	...
	public function Traitement_masse()
	{
		...
		if($this->Initialiser_tous_fichier_DDRT())
		{
			...
			$this->document_param['annee']="2018";
			$this->document_param['DATE_SCAN_SQL']= "";
			$this->document_param['DATE_SCAN_DEBUT_SQL']="";
			$this->document_param['DATE_SCAN_FIN_SQL']="";
	...
	*/
	...
	public function Get_num_candidat_nouvelle_appli($document_ged, $type_document)
	{
		$id_candidat = "";
		$DOSSIER = $document_ged["DOSSIER"];
		$sql = "SELECT MAPPING.id_defendeur FROM dbo.requete_ctx as REQ
				LEFT JOIN requete_ctx_grief as GRIEF ON (REQ.id_requete = GRIEF.id_requete)
                LEFT JOIN requete_ctx_defendeur as MAPPING ON (REQ.id_requete = MAPPING.id_requete_ctx)
	...
	public function Process_decision_mutualiser_nouvelle($metadonnee, $type_document)
	{
		$sql = " SELECT REQ.ref_ctx,GRIEF.chk_grief_financier FROM dbo.requete_ctx as REQ
					LEFT JOIN requete_ctx_grief as GRIEF ON (REQ.id_requete = GRIEF.id_requete)
                    LEFT JOIN requete_ctx_defendeur as mapping on (mapping.id_requete_ctx = REQ.id_requete)
					where REQ.id_scrutin = '".$metadonnee["N_scrutin"]."' AND mapping.id_defendeur = '".$metadonnee["N_candidat"]."'
    ...
    public function get_sql_details_ancien_application_v2($document_ged, $type_document)
	{
		...
		$sql = "SELECT top 1
			...
			WHERE
				...
				AND FILIERE_DH.date_debut < right(TCR.DatePassCCFP, 4)+SUBSTRING(TCR.DatePassCCFP, 4, 2)+left(TCR.DatePassCCFP, 2)
				AND (FILIERE_DH.date_fin > right(TCR.DatePassCCFP, 4)+SUBSTRING(TCR.DatePassCCFP, 4, 2)+left(TCR.DatePassCCFP, 2) OR FILIERE_DH.date_fin is null)
			";
	...
	public function Convertir_pdf_text_ancien_appli($document_ged, $type_document, $metadonnee, $DETAILS)
	{
		...
		generer_pdf_text_file($this->conn_nouvelle_appli,$IdCandidat,$id_decision,$etape,$chemin_fichier_pdf,$DETAILS); //Par EA le 19 09 2018 - Je remets la genration du pdf texte (et non plus sa récupération dans zz_decisions_pdf)
