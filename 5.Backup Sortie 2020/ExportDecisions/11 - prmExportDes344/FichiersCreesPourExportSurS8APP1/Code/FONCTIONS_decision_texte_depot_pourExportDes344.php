...
class Outil_consultation_sp
{
	...
	*/
	...
	public function Process_mutualises_documents_ged()
	{
		...
		else //on exporte toute une année
		{
			if ($this->document_param['annee'] == 2010)
			{
				$w[] = "FD_09A9C5FE IN (894, 941, 758, 839, 843)"; //On filtre que les sur candidats qui ont dateCCFP = NULL
			}
			elseif ($this->document_param['annee'] == 2011)
			{
				$w[] = "FD_09A9C5FE IN (1545, 1813, 1838, 2141, 2277, 2568, 2719, 2936, 3142, 3189, 3200, 3397, 3420, 3455, 3627, 3751, 3808, 3846, 3849, 3894, 3909, 4137, 4168, 4223, 4231, 4287, 4298, 4313, 4324, 4332, 4386, 4435, 4453, 4504, 4512, 4522, 4643, 4644, 4654, 4670, 4735, 4883, 4960, 5001, 5013, 5014, 5016, 5055, 5095, 5119, 5295, 5381, 5417, 5515, 5523, 5530, 5583, 5610, 5649, 5659, 5873, 5875, 5885, 5897, 6045, 6055, 6123, 6132, 6172, 6185, 6347, 6414, 6447, 6489, 6562, 6645, 6685, 6711, 6968, 6987, 7127, 7316, 7418, 7501, 7568, 7607, 7684, 7696, 7774, 7846, 7862, 7890, 7893, 7907, 8067, 8145, 8208, 8231, 8241, 8365, 8388, 8412, 8464, 8488, 8524, 8537, 8571)"; //On filtre que les sur candidats qui ont dateCCFP = NULL
			}
			elseif ($this->document_param['annee'] == 2012)
			{
				$w[] = "FD_09A9C5FE IN (2, 336, 353, 846, 852, 857, 1399, 1469, 1480, 1497, 1670, 1830, 1904, 1918, 1980, 1982, 2090, 2144, 2152, 2213, 2266, 2319, 2340, 2403, 2423, 2459, 2463, 2464, 2616, 2651, 2655, 2656, 2697, 2718, 2720, 2731, 2871, 2880, 2887, 2943, 3024, 3032, 3036, 3067, 3221, 3240, 3250, 3306, 3343, 3530, 3557, 3632, 3701, 3779, 3815, 3832, 3851, 3857, 3863, 3983, 4001, 4104, 4141, 4154, 4176, 4309, 4396, 4403, 4411, 4436, 4484, 4561, 4886, 4903, 4963, 5037, 5093, 5095, 5142, 5165, 5276, 5295, 5381, 5641, 5648, 5743, 5785, 5807, 5861, 5875, 5890, 5899, 5924, 6019, 6023, 6033, 6111, 6159, 6169, 6187, 6276, 6305, 6469, 6529, 6531, 6565, 6566, 6568, 6605, 6643, 6675, 6715, 6759, 6875, 6901, 6907, 6912, 6919, 6927, 6930, 6957, 6965, 6990, 7015)"; //On filtre que les sur candidats qui ont dateCCFP = NULL
			}
			elseif ($this->document_param['annee'] == 2013)
			{
				$w[] = "FD_09A9C5FE IN (43, 61, 72)"; //On filtre que les sur candidats qui ont dateCCFP = NULL
			}
			elseif ($this->document_param['annee'] == 2014)
			{
				$w[] = "FD_09A9C5FE IN (175, 351, 355, 507, 508, 724, 749, 871, 885, 896, 917, 921, 1099, 1118, 1132, 1148, 1212, 1230, 1233, 1237, 1254, 1278, 1301, 1329, 1356, 1389, 1399, 1401, 1429, 1444, 1451, 1645, 1691, 1693, 1706, 1712, 1713, 1718, 2050, 2218, 2356, 2357, 2368, 2370, 2395, 2415, 2471, 2522, 2598, 2726, 2782, 2806, 2816, 2894, 2895, 2915, 2943, 3012, 3027, 3096, 3126, 3205, 3270, 3294, 3481, 3508, 3515, 3540, 3548, 3589, 3641, 3790, 3872, 3883, 3904, 3938, 3950, 4004, 4007, 4069, 4181, 4255, 4392, 4599, 4663, 4705, 4813, 4862, 4898, 4970, 4984, 5175, 5238, 5344, 5347, 5365, 5388, 5448, 5481, 5654, 5717, 5763, 5775, 5784, 5789)"; //On filtre que les sur candidats qui ont dateCCFP = NULL
			}	
		}
			...
			WHERE
				...
				AND (--Filtrer une partie des doublons, et surtout déterminer le bon cm aumoment de l'élection
						(-- Cas où ((date debut et date fin renseignées) OU (date debut = 0 et date fin renseignee))
							FILIERE_DH.Date_debut <= CAST(SUBSTRING(Date1T,7,4)+SUBSTRING(Date1T,4,2)+SUBSTRING(Date1T,1,2) AS BIGINT)
							AND
							CAST(SUBSTRING(Date1T,7,4)+SUBSTRING(Date1T,4,2)+SUBSTRING(Date1T,1,2) AS BIGINT) <= FILIERE_DH.Date_fin
						)
						OR
						(-- Cas où date debut renseignée et date fin nulle
							CAST(SUBSTRING(Date1T,7,4)+SUBSTRING(Date1T,4,2)+SUBSTRING(Date1T,1,2) AS BIGINT) >= FILIERE_DH.Date_debut
							AND
